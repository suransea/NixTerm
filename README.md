...
NixTerm runs a selectable aarch64 NixOS or FreeBSD environment inside interpreted QEMU on stock iOS. SwiftTerm is connected directly to the guest serial console, so the guest kernel and shell own command parsing, PTYs, signals, pipes, jobs, and process management.
...
Select the guest resources before building:

```sh
nix run .#prepare-guest -- --guest nixos
nix run .#prepare-guest -- --guest freebsd
```
...
The build script stages only the selected guest, generates the Xcode project, signs the application and all embedded frameworks, and installs it with `devicectl`. NixOS uses the compact read-only SquashFS environment. FreeBSD starts from the official 14.4 arm64 UFS VM image with Zsh, Oh My Zsh, and a One Dark prompt preinstalled. It runs from persistent qcow2 layers and pairs each Quick Start memory snapshot with an untouched disk layer; disk changes are preserved and trigger one cold boot to rebuild the pair.
